...
class TourismAttachmentsController extends AppController
{

	public function __construct(Slim\Container $ci)
	{
		parent::__construct($ci);
		$this->table = 'tourism_attachments';
	 ...
	 */
	public function createPhoto($request, $response)
	{
		...
		if (!is_numeric($data_temp['tourism_id']) || isset($_FILES['file']) === false) {
			$handler = $this->ci->get('badRequestHandler');
			return $handler($request, $response);
		}
		
		$path = $this->attachment['photo_tourism'].$data_temp['tourism_id'];
	 ...
	 *  create new ticket request attachments data
	 */
	public function createVideo($request, $response)
	{
		$data_temp = $request->getParsedBody();
		$column = $this->checkColumn($this->db['database']['dbname'], $this->table);
		...
		$path = $this->attachment['video_tourism'].$data_temp['tourism_id'];
	...
	/**
	 *  getByTicket method
	 *  get ticket attachment by ticket id
	 */
	public function getByTourism($request, $response, $args)
	{
		$tourism_id = $args['tourism_id'];
		
		$params = $request->getQueryParams();
		$column = $this->checkColumn($this->db['database']['dbname'], $this->table);
		$column = array_diff($column, ['path']);
		$sort = ['ASC', 'DESC'];
		...
		$result = $this->parentGetAll($this->table, $clause, $table_condition, $column);
	...
	public function delete($request, $response, $args)
	{
        ...
        if ($file['is_photo'] == 1) {
            $path = $this->attachment['photo_tourism'].$file['tourism_id'].'/'.$file['filename'];
        }

        if ($file['is_video'] == 1) {
            $path = $this->attachment['video_tourism'].$file['tourism_id'].'/'.$file['filename'];
        }
